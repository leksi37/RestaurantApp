...
public class Chef {
    ...
    private void init(ChefViewModel vm){
        this.viewModel = vm;
        viewModel.addListener("New order for the chef", this::setItems);
    }
    ...
    private void setItems(PropertyChangeEvent propertyChangeEvent) {
        ...
        for (int i = 0; i < itemsFromOrder.size(); ++i) {
            HBox box = new HBox();
            box.setId(itemsFromOrder.get(i).getTableId());


            Label name = new Label(itemsFromOrder.get(i).getItems().get(i).getName());
